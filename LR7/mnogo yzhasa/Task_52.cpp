...
int main() {
    ...
    // Запись номеров бочек, из которых пили только мертвые рабы после 48 часов, в массив
    ...
    // // Проверка состояния рабов через 24 часа и определение отравленной бочки
    // int poisonedBarrelDetected = 0;
    // for (int i = 0; i < 5; ++i) {
    //     if (!slaves[i].alive) {
    //         poisonedBarrelDetected += (1 << i);
    //     }
    // }

    //std::cout << "номер отравленной бочки: " << poisonedBarrelDetected << std::endl;
    ...
    // Вывод результатов
    // for (const auto& slave : slaves) {
    //     std::cout << "Раб " << slave.name << " (ID: " << slave.id << ") будет пить из бочек: ";
    //     for (const auto& b : slave.barrels) {
    //         std::cout << b << " ";
    //     }
    //     std::cout << std::endl;
